...
const state = {};
state._getTraversalTemplate = function (el, refs, refName, refSpecial) {
  if (!el || !el.childNodes) {
    return false;
  }
  ...
  for (let i = 0; i < el.childNodes.length; i++) {
    ...
    state._getTraversalTemplate(ref, refs, refName, refSpecial);
  }
}
const Ref = {};
...
Ref.renderAttr = function (ref, state, { key, replaceAttr, handle }) {
  if (!ref.getAttributeNames) {
    return false;
  }
  const attrs = ref.getAttributeNames();
  for (let i = 0; i < attrs.length; i++) {
    const attrKey = attrs[i];
    if (!attrKey.match(key)) {
      continue;
    }
    const template = `{{${ref.getAttribute(attrKey)}}}`;
